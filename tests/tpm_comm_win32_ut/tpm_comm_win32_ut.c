...
//MOCKABLE_FUNCTION(, TBS_RESULT, Tbsi_Context_Create, PCTBS_CONTEXT_PARAMS, pContextParams, PTBS_HCONTEXT*, phContext);
//MOCKABLE_FUNCTION(, TBS_RESULT, Tbsi_GetDeviceInfo, uint32_t, size, PVOID, info);
//MOCKABLE_FUNCTION(, TBS_RESULT, Tbsip_Context_Close, TBS_HCONTEXT, hContext);
//MOCKABLE_FUNCTION(, TBS_RESULT, Tbsip_Submit_Command, TBS_HCONTEXT, hContext, TBS_COMMAND_LOCALITY, Locality, TBS_COMMAND_PRIORITY, Priority, const PCBYTE*, pabCommand, UINT32, cbCommand, PBYTE*, pabResult, UINT32*, pcbOutput);
...
TBS_RESULT my_Tbsi_GetDeviceInfo(UINT32 size, PVOID info)
{
    (void)size;
...
TBS_RESULT my_Tbsi_Context_Create(PCTBS_CONTEXT_PARAMS pContextParams, PTBS_HCONTEXT* phContext)
{
    (void)pContextParams;
...
TBS_RESULT my_Tbsip_Context_Close(TBS_HCONTEXT hContext)
{
...
BEGIN_TEST_SUITE(tpm_comm_linux_ut)
    ...
    TEST_FUNCTION(tpm_comm_create_succeed)
    {
        ...
        //STRICT_EXPECTED_CALL(Tbsi_Context_Create(parms, IGNORED_PTR_ARG))
        //    .IgnoreArgument_pContextParams();
